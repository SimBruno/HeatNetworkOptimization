...
In this section, the task is to compute the thermodynamic properties as well as the mass flows and heat exchanged on each heat exchangers of all the streams within each subsystem of the dairy factory. The sections that are going to be analyzed are the following:
...
In addition, in order to add value to the waste/outputs of some of the subsystems, 2 additionnal systems will be analysed:
...
Because of the mass flow conservation:
...
$\textit{Note}$: However, after completing calculations, it appears the flows entering HX3 were not respecting the condition ($T_{cold,in}<T_{hot,out}$). This comes from the fact the hot inlet flow e20 was too low. 
One solution could have been to reduce e18 mass flow. However, the problem was solved by first determining all mass and heat balance for the three evaporator before handling the HX. This decision (at first validated by some TAs) made it was impossible to change e18 without changing the whole system (it was discovered to late in the semester). 
Therefore to solve in a - hopefully more original and-  convenient way this issue, it is assumed a steam flow can be added to make e20 reach a certain value such that $T_{cold,in}<T_{hot,out}$ in the HX3. 
If it was to be reperformed the mass and heat balance of such system would be determined by starting with evap1, then HX4, then evap2, then HX3, etc, until HX1. This would let more flexibility to adapt the system to the temperature constraints. 
...
- $cp_{\text{glycol}}$ = 2294 kJ/kg.K from heat capacity tables at 270 K 
...
m_cip11 = m_cipglass
```
Thus, there is `r Q_shower` $kW$ of heat used at each shower and bottle flow rate is `r round(m_cipglass,1)` $kg/s$.
...
By the use of an air mass flow, 500kW of heat are removed from a cold storage. Simple thermodynamic calculations, as reported in the previous subsytems, enable to get the air mass flow must be of 100kg/s.
...
This subsytem analyses how are treated the waste water from the evaporation process. No heat Exchanger are present so this system won't be really useful for further energy saving analyses.

For this process we just have to focus on computing liquid and dry matter mass flows for each of the 4 flows ($dw_{tot}=dw_{dry}+dw_{liq}$). The discharge will only contains liquid water whereas the sludge is a mixture of 13.3wt% dry matter and 86.7wt% water (@shiDairyProcessingSludge2021). From this paper, the following equations enables to deduce the waste water produced (WP) as well as the sludge produced (SP):
...
The heat exchanged during the evaporation is found to be of 579.4kW.
...
The heat exchanged in the HXRed between flows 20 and 21 is 28.9kW.
