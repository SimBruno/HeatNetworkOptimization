...
In this section, the task is to compute the thermodynamic properties as well as the mass flows and heat exchanged at each heat exchanger of all the streams within each subsystem of the dairy factory. The sections that are going to be analyzed are the following:
...
We also consider the centrifuge separates an ideal mix, therefore:
...
In this subsection the heat flow equations for each element of the pasteurisation are indicated. All variables are defined on the pasteurization scheme or in the mass flow and heat capacities sections. 
...
The aim of this section is to increase the fat proportion from $11.7\% wt.$ to $50\% wt.$ by evaporating water. It is composed of heat-exchangers (HEX), 3 evaporators, 2 splitters and a processing module. 
...
One solution could have been to reduce the mass flow e18. However, the problem was solved by first determining all mass and heat balance for the three evaporator before handling the HX. This decision (at first validated by some TAs) made it impossible to change e18 without changing the whole system , and it was discovered too late in the semester. 
...
These two little subsystems are quite simple. 
...
This subsytem treats the waste water from the evaporation process. No heat Exchanger is present so this system won't be really useful for further energy saving analyses.
