...
```{r compute_pasteurisation, echo = F, eval = T}
...
|--------|---|-|
| $c_{p,Fresh~Milk}$ | `r Cp_fmilk` | J/(kg.K)|
| $Q_{refrigeration}$ | `r Q_refrigeration/1000` |kJ/s|
| $Q_{past1}$ | `r Q_past1/1000` |kJ/s|
| $T_{pastc}$ | `r T_past_c - 273.15` |°C|
| $T_{pasta}$ | `r T_past_a - 273.15` |°C|
| $T_{pastd}$ | `r T_past_d - 273.15` |°C|
| $Q_{past2}$ | `r Q_past2/1000` |kJ/s|
| $Q_{past3}$ | `r Q_past3/1000` |kJ/s|
| $Q_{past4}$ | `r Q_past4/1000` |kJ/s|
| $T_{crpast\_ a}$ | `r T_crpast_a - 273.15` |°C|
| $T_{crpast\_ c}$ | `r T_crpast_c - 273.15` |°C|
| $Q_{past5}$ | `r Q_past5/1000` |kJ/s|
| $T_{crpast\_ x}$ | `r T_crpast_x - 273.15` |°C|
| $Q_{past6}$ | `r Q_past6/1000` |kJ/s|
| $Q_{past7}$ | `r Q_past7/1000` |kJ/s|
...
The mass flow $m_1$ is a given data and the mass flows $m_{14}$, $m_{15}$ and $m_{16}$ are parameters that can be manually changed in order to get a reasonable system. Knowing those 4 mass flows and the target mixture, the fat content at each point can be computed.
...
EXPLIQUER LA FONCTION ET VOIR POUR ENLEVER HX4 (MASS FLOW TROP FAIBLE) ET REMPLACER PAR UN ELECTRIQUE HEATER AVANT HX1
