...
In the following section, exergy analysis will first be performed to assess how far from ideal case the system is. The idea is to assess where energy is not useful and where it is, and thus evaluate where improvment could be performed.
In a second part, the energy operating costs will be computed to assess how much cooling and heating utilities cost. 
...
## Energy bill
In this section OPEX will be computed to assess how much cooling and heating utilities cost. The OPEX is dependent on both heating./cooling demand and the price of the utilities. The price of electricity and natural gas in France and Germany, for non-household consummers, are given in the following table and are taken from the following sources @noauthor_electricity_nodate and @noauthor_natural_nodate:
...
The OPEX cost is then computed as follow:

$$
OPEX(M€/y)= [Q_{heat}(kW)\cdot price_{gas}(€/kWh) + Elec(kW)\cdot price_{elec}(€/kWh)] \cdot t_{\text{op}} (h/y)  \cdot 10^{-6} (M€/€)
$${#eq-opex}

Thus, only heat exchanger or evaporator which requires the use of external cold or hot utilities are taken into account in the OPEX. The other heat exchangers are not considered since they are internal and thus selfsustainable.
