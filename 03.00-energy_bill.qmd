...
```
# Exergy analysis and Energy bill (Tasks 2 & 3) 

In the following section, an exergy analysis will be performed to assess how far from the ideal case the system is. The idea is to assess where energy is not useful and where it is, and thus evaluate where improvement could be performed.

In a second part, the energy operating costs will be computed to assess how much the cooling and heating utilities cost. 

## Exergy analysis

To evaluate the losses and inefficiencies in the dairy production, an exergy analysis is performed. An exergy loss is the difference between the exergy entering a unit and the exergy leaving the unit. This exergy analysis includes calculating the losses for each component experiencing heat transfer, covering both the pasteurization and evaporation processes. The ambient temperature is set at $T_a = 25°C + 273K = 293K$.
...
In the heat exchanger dedicated to Past 2, exergy is dissipated through the two milk streams.
$$
...
In the heat exchanger dedicated to Past 5, exergy is dissipated through the two cream streams.
$$
L_{past5\_1} = \dot{m}_{int}c_{p_{cream}}\left((T_{cream} - T_{crpast\_a}) - T_a \ln\left(\frac{T_{cream}}{T_{crpast\_a}}\right)\right)
$$
$$
L_{past5\_2} = \dot{m}_{cream}c_{p_{cream\_mixed}}\left((T_{crpast\_b} - T_{crpast\_c}) - T_a \ln\left(\frac{T_{crpast\_b}}{T_{crpast\_c}}\right)\right)
$$
$$
...
$$
$$
L_{past6\_2} = \dot{m}_{steam\_{past6}}c_{p_{steam}} (\Delta h_6 - T_a \Delta s_6)
$$
$$
...
In the heat exchanger dedicated to the mixer, exergy is dissipated through the cream and thickener streams.
$$
...
To visually represent the exergy flows in the system, a Sankey diagram is plotted. It shows the exergy in-flows and out-flows with respect to the factory. 
...
####  Exergy Efficiency Pasteurisation

The efficiency of the pasteurisation process can be calculated with the following formula:

$$
\eta _{ex}=\frac{\dot{E}_{in}}{\dot{E}_{out}}=1-\frac{\dot{L}}{\dot{E}_{in}}
$$

As a result an exergy efficiency of 70.3% is obtained.

### Evaporation 

The second process to be analysed is the evaporation process. Heat transfers occur in the heat exchangers, heat evaporators and splitters. 

Since the splitters are isenthalpic no exergy is lost. 

The exergy losses for the heat exchangers and the heat evaporators can be calculated with the following general equation: 
...
There are five heat exchangers in the system: P1, P2, P3, P4 and P5. The exergy that is lost for each of the heat exchangers can be calculated with the following equations:
$$
L_{heat1} = m_{11}(h_{11} - T_a s_{11}) - m_{12}(h_{12} - T_a s_{12}) + m_1(h_1 - T_a s_1) - m_2(h_2 - T_a s_2)
$$
$$
L_{heat2} = m_2(h_2 - T_a s_2) - m_3(h_3 - T_a s_3) + m_{16}(h_{16} - T_a s_{16})- m_{ex2}(h_{ex2} - T_a s_{ex2})
$$
$$
L_{heat3} = m_3(h_3 - T_a s_3) - m_4(h_4 - T_a s_4) + m_{20}(h_{20} - T_a s_{20})-m_{ex3}(h_{ex3} - T_a s_{ex3})
$$
$$
L_{heat4} = m_4(h_4 - T_a s_4) - m_5(h_5 - T_a s_5) + m_{19}(h_{19} - T_a s_{19})- m_{ex4}(h_{ex4} - T_a s_{ex4})
$$
$$
L_{heat5} = m_12(h_12 - T_a s_12) - m_13(h_13 - T_a s_13) + m_{gly}c_{p_{gly}}\left((T_{in\_gly} - T_{out\_gly}) - T_a \ln\left(\frac{T_{in\_gly}}{T_{out\_gly}}\right)\right)
$$

#### Heat Evaporator

There are three heat evaporators in the system: EVAP 1, EVAP 2 and EVAP 3.
The exergy that is lost for each of the heat evaporators can be calculated with the following equations:
$$
...
#### Total
The amount of exergy that is lost in the evaporation process is calculated by adding together all the exergy losses of the heat exchangers. 
$$
...
The following Table presents a summary of all the calulated exergy losses of the evaporation process:
...
m_ex2=0.8
h_ex2=122642.15967891106
s_ex2=426.49113053055

m_ex3=0.9
h_ex3=206397.68253099348
s_ex3=694.6484331540266

m_ex4=0.31
h_ex4=270918.9238699816
s_ex4=890.092650494589
...
L_heat2= -m2*(h2-Ta*s2)+m3*(h3-Ta*s3)-m16*(h16-Ta*s16)+m_ex2*(h_ex2-Ta*s_ex2)
L_heat3=-m3*(h3-Ta*s3)+m4*(h4-Ta*s4)-m20*(h20-Ta*s20)+m_ex3*(h_ex3-Ta*s_ex3)
L_heat4=-m4*(h4-Ta*s4)+m5*(h5-Ta*s5)+m15*(h15-Ta*s15)+m_ex4*(h_ex4-Ta*s_ex4)
L_heat5_1=m12*(h12-Ta*s12)-m13*(h13-Ta*s13)
...
L_heat5_2=m_gly*Cp_gly*((T_glyin-T_glyout)-T_a*np.log(T_glyin/T_glyout))
L_heat5=L_heat5_1+L_heat5_2
...
Whey_out=m22*(h22-Ta*s22)/1000
Wastewater=m23*(h23-Ta*s23)/1000
steam_in=m23*(h24-Ta*s24)/1000
concentrated_milk=m13*(h13-Ta*s13)/1000

# Dataframe for Exergy Loss Evaporation

Exergy_losses  = 'kW'

Variables_Names = ['L_heat1','L_heat2','L_heat3','L_heat4','L_heat5','L_eva1','L_eva2','L_eva3','L_tot_eva']
Variables_Values = np.divide([L_heat1,L_heat2,L_heat3,L_heat4,L_heat5,L_eva1,L_eva2,L_eva3,L_tot_eva],1000)
Variables_Units =  Exergy_losses
...
The following Figure shows the Sankey diagram for the Evaporation process. 
```{r sankey-function2, echo = T, eval = T, warning = F, message=F}
source("./sankey_ipese.R") # introduce the sankey function from the source file
df = read.csv('Sankey_Evaporation.csv',sep = '\t') # replace the atom_balance.csv with your exergy analysis results
sankey_ipese(df = df, unit = 'kW', title = 'Sankey diagram Evaporation') # replace the unit and title here
```
#### Exergy Efficiency Evaporation

Based on the obtained results an exergy efficiency of 46% is obtained.

### Suggested modifications

From the previous calculations, one can see that the total exergy lost to the environment from the pasteurization and evaporation processes is more than 500 kW. This value indicates that the inefficiencies in both processes are very large. For the pasteurization process, the exergy loss is especially significant in Past 6, as steam enters and leaves the system. In the evaporation part, the exergy loss is particularly high in Evaporator 1 and Evaporator 2.

To decrease the overall exergy losses in the system, different strategies can be implemented. Since the exergy efficiencies are so large the best option would be to improve the general energy efficiency by upgrading to more energy-efficient equipment and using insulation.

## Energy bill
In this section the OPEX will be computed to assess how much the cooling and heating utilities cost. The OPEX is dependent on both the heating and the cooling demand and the price of the utilities. 

The price of electricity and natural gas in France and Germany, for non-household consumers, are given in the following table and are taken from @noauthor_electricity_nodate and @noauthor_natural_nodate:
...
A few assumptions were used to compute the energy bill:

- The factory is running 95% of the time (8322 hours per year)
- COP of the refrigeration system is 3.5
- All water is used in closed loops. Its cost is not taken into account in the OPEX but will be consideered for the calculations of the CAPEX.
...
The electricity consumption required for the cooling is computed as follows:

$$
Elec(kW)= \frac{Q(kW)}{COP}
$${#eq-cool_elec}

And for the heating:

$$
Q_{heat}(kW)= Q_{fuel} = \frac{Q(kW)}{\eta_{steamprod}}
$${#eq-heat_ng}

The OPEX cost is then computed as follows:

$$
OPEX(M€/y)= [Q_{heat}(kW)\cdot price_{gas}(€/kWh) + Elec(kW)\cdot price_{elec}(€/kWh)] \cdot t_{\text{op}} (h/y)  \cdot 10^{-6} (M€/€)
$${#eq-opex}

Thus, only heat exchangers or evaporators which require the use of external cold or hot utilities are taken into account in the OPEX. The other heat exchangers are not considered since they are internal and thus selfsustainable.
