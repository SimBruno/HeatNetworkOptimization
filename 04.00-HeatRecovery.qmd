...
We fix crpast_b to 75°C so we don't reoptimize on past5
...
print('h2 =', round(h2_hx, 3), 'kJ/kg')

cp1_hx = State_Cold_1_hx["cpmass"]
cp2_hx = State_Cold_2_hx["cpmass"]

print('cp1 =', round(cp1_hx, 3), 'kJ/kg.K')
print('cp2 =', round(cp2_hx, 3), 'kJ/kg.K')

slope1_hx = 1/(m1_hx*cp1_hx)
slope2_hx = 1/(m2_hx*cp2_hx)

print('slope1 =', slope1_hx, 'K/J')
print('slope2 =', slope2_hx, 'K/J')
...
print('dTmin =', round(dTmin_hx, 3), 'K at h =', round(h_hx[min_index_hx], 3), 'kJ/kg')
...
dt_hx = np.linspace(5, 60)

for t in dt_hx:
    CAPEX_hx, OPEX_fr_hx, OPEX_ger_hx, TOTEX_fr_hx, TOTEX_ger_hx, Area_hx = Dtmin(t, T1_hx, T1v_hx, T2v_hx, T2_hx, m1_hx, m2_hx, cp1_hx, cp2_hx, 'right', 'HX')[0:6]
    CAPEXv_hx.append(CAPEX_hx)
    OPEX_frv_hx.append(OPEX_fr_hx)
    OPEX_gerv_hx.append(OPEX_ger_hx)
    TOTEX_frv_hx.append(TOTEX_fr_hx)
    TOTEX_gerv_hx.append(TOTEX_ger_hx)
    Areav_hx.append(Area_hx)

#plot the results with respect to the dt values

# Create traces for each cost component
trace_capex = go.Scatter(x=dt_hx, y=CAPEXv_hx, mode='lines', name='CAPEX', line=dict(color='black'))
trace_opex_france = go.Scatter(x=dt_hx, y=OPEX_frv_hx, mode='lines', name='OPEX (France)', line=dict(color='blue'))
trace_opex_germany = go.Scatter(x=dt_hx, y=OPEX_gerv_hx, mode='lines', name='OPEX (Germany)', line=dict(color='red'))
trace_totex_france = go.Scatter(x=dt_hx, y=TOTEX_frv_hx, mode='lines', name='TOTEX (France)', line=dict(color='blue', dash='dash'))
trace_totex_germany = go.Scatter(x=dt_hx, y=TOTEX_gerv_hx, mode='lines', name='TOTEX (Germany)', line=dict(color='red', dash='dash'))
...
# Mark the minima points for both France and Germany
min_index_fr_hx = np.argmin(TOTEX_frv_hx)
fig = fig.add_trace(go.Scatter(x=[dt_hx[min_index_fr_hx]], y=[TOTEX_frv_hx[min_index_fr_hx]], mode='markers', marker=dict(color='blue'), name='Min TOTEX (France)'))

min_index_ger_hx = np.argmin(TOTEX_gerv_hx)
fig = fig.add_trace(go.Scatter(x=[dt_hx[min_index_ger_hx]], y=[TOTEX_gerv_hx[min_index_ger_hx]], mode='markers', marker=dict(color='red'), name='Min TOTEX (Germany)'))
...
dtmin_fr_opt_hx = dt_hx[min_index_fr_hx]
dtmin_ger_opt_hx = dt_hx[min_index_ger_hx]
area_fr_opt_hx = Areav_hx[min_index_fr_hx]
area_ger_opt_hx = Areav_hx[min_index_ger_hx]
CAPEX_fr_opt_hx = CAPEXv_hx[min_index_fr_hx]
CAPEX_ger_opt_hx = CAPEXv_hx[min_index_ger_hx]
OPEX_fr_opt_hx = OPEX_frv_hx[min_index_fr_hx]
OPEX_ger_opt_hx = OPEX_gerv_hx[min_index_ger_hx]
TOTEX_fr_opt_hx = TOTEX_frv_hx[min_index_fr_hx]
TOTEX_ger_opt_hx = TOTEX_gerv_hx[min_index_ger_hx]

HX_opt_fr = pd.DataFrame({'dTmin_fr[K]': dtmin_fr_opt_hx, 'Area_fr[m^2]': area_fr_opt_hx, 'CAPEX_fr[€/yr]': CAPEX_fr_opt_hx, 'OPEX_fr[€/yr]': OPEX_fr_opt_hx, 'TOTEX_fr[€/yr]': TOTEX_fr_opt_hx}, index=[0])
HX_opt_fr = HX_opt_fr.round(2)
HTML(HX_opt_fr.to_html(index=False))

HX_opt_ger = pd.DataFrame({'dTmin_ger[K]': dtmin_ger_opt_hx, 'Area_ger[m^2]': area_ger_opt_hx, 'CAPEX_ger[€/yr]': CAPEX_ger_opt_hx, 'OPEX_ger[€/yr]': OPEX_ger_opt_hx, 'TOTEX_ger[€/yr]': TOTEX_ger_opt_hx}, index=[0])
...
NB: We could have tried the same by preheating the CIP2 for it to be closer to CIP3 temperature of 80°C. However the gap would have been lowered (only 20°C difference compared to 35°C for the chosen solution) and thus the heat recovery would have been less efficient.
...
- T_cip6=75°C > T_cip1=25°C

The new HX will placed such that CIP6 flow is first used to preheat CIP1 to CIP1' (with T_cip1' > T_cip1) and the exiting flows called CIP6' (such that T_cip6'< T_cip6) is then used to be cooled where CIP6 was previously. Hence we would have a system with less cooling required since T_cip6'< T_cip6, and with also less heating required since T_cip1' > T_cip1.

**AAAAAAADDDDDDDD ILLLLLLUUUSSSSSTRTATION** 
...
print('h1 =', round(h1_cip, 3), 'kJ/kg')
print('h2 =', round(h2_cip, 3), 'kJ/kg')

cp1_cip = State_Cold_1_cip["cpmass"]
cp2_cip = State_Cold_2_cip["cpmass"]

print('cp1 =', round(cp1_cip, 3), 'kJ/kg.K')
print('cp2 =', round(cp2_cip, 3), 'kJ/kg.K')
...
We can observe that the optimum dT is 45K which brings CIP1 to 70°C instead of 25°C. Thus compared to the case without optimization CIP2 will be hotter than 60°C and thus the heat exchanger between CIP2 and CIP3/10 will require less steam to heat up until 80°C.
