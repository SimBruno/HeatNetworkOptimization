...
Then, in task 5, a first look at the global optimization of the entire factory will be achieved by plotting the Hot and Cold Composite Curves as well as the Grand Composite Curve. This will then allow us to determine a crucial temperature, the pinch point, from which a further optimization can be achieved in task 7 with the HEN design.
...
The goal of this part is to determine an optimized $\Delta T_{min}$ such that costs are minimized for the considered subsystems. 
...
Now the same as above is performed once again except for past 5. This time because both past 1 and past 5 are connected it is assumed that $T_{cold,in,past5}=T_{hot,out,past1}$
...
Now, let us compute the output temperature of the mixer.
...
Let us try to apply the method used for the pasteurization:
...
From the composite curves we are then able to extrapolate the energy requirements, the heat recovery and the utilities. By separating the graph in three portions with two green dotted lines we show the boundaries of the utilities and the heat recovery.

We can then use the graph of the grand composite curve to determine the temperature of the pinch points. These are the points where the heat load is equal to 0, meaning that the heats of the corrected cold and hot streams are the same in these points.
