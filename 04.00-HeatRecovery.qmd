...
After having determined the energy and mass exchanged in the entire subsystems (task 1), performed an exergy analysis (task2) and computed the energy bill of the factory (task 3), it is now time to see where improvement can be achieved and tried different approach to optimize the system

First, in task 4, optimization of the minimum temperature difference ($\Delta T$) in heat exchangers, will be performed. The goal will be to see if in 2 processes (namely Pasteurization and Cleaning In Place), it is possible to optimize the $\Delta T_{min}$ of some of the HX to reduce the energy bill. 

Then, in task 5, a first look at global optimization of the entire factory will be achieved by ploting the Hot and Cold Compiste curves as well as the Grand Composite Curve. This will then enable to determine a crucial Temperature, the pinch point, from which a further optimization can be achieved in task 7 with the HEN design.
...
- Determine the area of the HX in term of the $\Delta T_{min}$
...
The idea is too be able to determine if this optimized $\Delta T_{min}$ enables costs reduction compared to the assumed ∆Tmin of 5K in the Mass/Energy Balances and Energy Bill.
...
#### Initialisation
First, the T-h diagram of the cold and hot streams is plotted to determined if the $\Delta T_{min}$ is on the left or right of the diagram (i.e. between $T_{hot,in}$ and $T_{cold,out}$ or between $T_{hot,out}$ and $T_{cold,in}$). This will influence in the following formulas of CAPEX.
...
- Water consumption: however, water is considered as being used in a closed loop. Therefore, the cost in not considered in operation cost. It is also neglected in CAPEX since the cost is small compared to the heat exchangers cost.

### Past 5 only

Here, Past 5 heat exchanger cost is optimized with respect to $\Delta T_{min}$. In the following code, 1 stands for cold stream, and 2 for hot stream. When there is a T_#number, it is related to the entrance of the stream and _prime relate to the exiting. The _Number stands for the past number. 
...
The T-h diagram shows that the $\Delta T_{min}$ is on the right of the curves. This determines the equations to use for the CAPEX.
...
The optimisation gives a $\Delta T_{min}$ for past 5 of 6.3K leading to an area of 5.21$m^2$ and a larger TOTEX in Germany than France because of the higher price of electricity in Germany (both CAPEX are the same but OPEX vary). Compared to case where only past5 were optimized, it presents a higher $\Delta T_{min}$ but a lower Area and thus lower costs. Considering only the costs of past5, it may seem more interesting to optimize past1 and past5 simultaneously. However one have to look at the whole picture and don't forget that past1 was also optimized (thus consider total costs of both past1 and past5).

#### Both costs combined 

Finaly, let's combine both costs from past 1 and past 5 to get the overall CAPEX, OPEX, and TOTEX for the two pasteurizers. This total costs will then be comapred to the energy bill without optimization over $\Delta T_{min}$. 
...
The goal is to increase the thickener temperature entering in the mixer with the heat excess of the pasterized milk going out of the system. Considering the huge mass flow difference, it can be assumed that $T_{past,d1} \approx T_{past,d2} \approx T_{thickener,out} = 10.8°C$. To compute the cp of the thickener, the function mixture is used, assuming the thickener is composed of 100% fat. With all this information, the heat exchanged in the recovery HEX can be computed.
...
Now, let's compute the output temperature of the mixer.

- $m_{crpast,x} = m_{thickener} + m_{crpast,a} = 0.07 + 0.48 = 0.55 kg/s$
- $T_{crpast,a} = 65.9°C$
- $T_{thickener} = 10.8°C$

For this the same equations for non-isothermal mixer used in mass & energy balance section are reused.
...
This leads to $T_{crpast,x}=65.9°C$. Compared to the original pasteurisation process, there is thus an increase of 12.40°C. This will reduced the heat require on the past6 process
...
Let's try to apply the method used for the pasteurization:
...
In this scenario the OPEX is dependent on the the cooling and heating HX of the CIP process. First  new flows are redetermined for m_cip1 and m_cip5 such that $m_{cip1} = m_{cip2}*\frac{T_{cip2}-T_{cip5}}{T_{cip1}-T_{cip5}}$ and $m_{cip5}=m_{cip2}-m_{cip1}$ where $T_{cip1}$=$T_{cip2}$=$T_{cold,out,newHX}$ is the new exiting temperature cip1' hotter than cip1 from the task2.
From this, $Q_{steam} = m_{cip2} \cdot cp1 \cdot (353-T_{cip2})$ and $Q_{cooling} = m_{hot,newHX} \cdot cp_{hot,newHX} \cdot (T_{hot,out,newHX}-303)$ 
...
In each case presented above, optimized $\Delta T_{min}$ have been selected to reduce cost of the considered system. But then even by selecting this minimum cost, is this heat recovery optimization really worth it? To check this, comparison will now be made with the non-optimized cases from the Energy Bill.
