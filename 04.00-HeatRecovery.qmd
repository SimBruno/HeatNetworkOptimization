# Pinch analysis (Tasks 4 & 5)
After having determined the energy and mass exchanged in the entire subsystems (task 1), performed an exergy analysis (task2) and computed the energy bill of the factory (task 3), it is now time to see where improvements can be achieved and to try some different approaches to optimize the system.

First, in task 4, an optimization of the minimum temperature difference ($\Delta T$) in the heat exchangers will be performed. The goal will be to see whether in 2 processes, namely Pasteurization and Cleaning In Place, it would be possible to optimize the $\Delta T_{min}$ of some of the HX to reduce the energy bill. 

Then, in task 5, a first look at the global optimization of the entire factory will be achieved by plotting the Hot and Cold Composite Curves as well as the Grand Composite Curve. This will then allow to determine a crucial temperature, the pinch point, from which a further optimization can be achieved in task 7 with the HEN design.

## Optimization of $\Delta T_{min}$
...
The goal of this part is to determine an optimized $\Delta T_{min}$ such that the economic costs are minimized for the considered subsystems. 

In all scenarios the following will be done:

- Determine the area of the HX depending on the $\Delta T_{min}$
- Determine the cost of the HX depending on the Area

The costs will be computed for both France and Germany and will be composed of CAPEX and OPEX.

The idea is to be able to determine whether this optimized $\Delta T_{min}$ enables a cost reduction compared to the assumed ∆Tmin of 5K in the Mass and Energy Balances and Energy Bill.

Four different scenarios will be considered:

- Past 5 only
- Past 1 + Past 5 combined
- An additional heat recovery for pasteurization
- Heat Recovery in the Cleaning In Place (CIP) process

The three first scenarios focus on the pasteurization process whereas the later is about the CIP process.

### General Method

#### Initialisation
First, the T-h diagram of the cold and hot streams is plotted to determined whether the $\Delta T_{min}$ is on the left or right side of the diagram, i.e. between $T_{hot,in}$ and $T_{cold,out}$ or between $T_{hot,out}$ and $T_{cold,in}$. This will affect the following formulas for the CAPEX.

In the case of $h_1 < h_2$, the equation of the T-h diagram is:
...
Then, the $\Delta T_{min}$ is graphically determined on the right or on the left as the minimum difference between the two curves.

If $\Delta T_{min}$ is on the right:
$$
...
If $\Delta T_{min}$ is on the left:
$$
...
- U, the total heat transfer coefficient, is assumed to be half of the heat transfer coefficient of water ($1000 W/m^2K$) since the heat exchanger's thickness is much smaller than 1 meter.
$$
U = \frac{\alpha}{2}
$${#eq-U}

- LMTD, the logarithmic mean temperature difference, is computed as follows:

$$
LMTD = \frac{(T_{h,in}-T_{c,out})-(T_{c,in}-T_{h,out})}{\ln(\frac{T_{h,in}-T_{c,out}}{T_{c,in}-T_{h,out}})}
$$

Knowing the heat exchanger area, the CAPEX can be computed as follows:
...
- $C_p$ is the cost of the heat exchanger, which is computed as follows: $$C_p = \frac{I_t}{I_{t,ref}} \cdot 10^{k_1+k_2 \cdot \log(A_{ex})}$${#eq-Cp}
...
- $e = 0.92 €/\$$ is the exchange rate between U.S. dollars and euros.

- $a_n$ is the annualization factor, which is computed as follows: $$a_n = \frac{i(1+i)^n}{(1+i)^n-1} $${#eq-a_n}
    - $i = 0.05$ is the interest rate (Slide 8, T4.2)
    - $n = 20$ is the lifetime of the heat exchanger

#### Operating cost (OPEX)

The operating costs depend on three factors:

- Natural gas consumption for heating: $$c_{heating} = c_{gas} \cdot Q_{HEX} \cdot t_{op}$${#eq-heating}
    - $c_{gas} = 0.08 €/kWh$ is the price of natural gas in France and in Germany
...
- Electricity consumption for cooling: $$c_{cooling} = c_{elec} \cdot \frac{Q_{HEX}}{COP} \cdot t_{op}$${#eq-cooling}
    - $c_{elec} = 0.12 €/kWh$ is the price of electricity in France
    - $c_{elec} = 0.21 €/kWh$ is the price of electrivity in Germany
    - $Q_{HEX}$ is the heat exchanged between the two streams
    - $COP = 3.5$ is the coefficient of performance of the heat pump

- Water consumption: however, water is considered as being used in a closed loop. Therefore, the cost is not considered in the operation costs. It is also neglected in the CAPEX since the cost is small compared to the heat exchanger's cost.

### Past 5 only

Here, the Past 5 heat exchanger's cost is optimized with respect to $\Delta T_{min}$. In the following code, 1 stands for cold stream, and 2 for hot stream. When there is a T_#number, it is related to the entrance of the stream and _prime relate to the exiting. The _Number stands for the past number. 
...
The T-h diagram shows that the $\Delta T_{min}$ is on the right of the curves. This determines the equations that will be used for the CAPEX.
...
This time only $T_{c,in}$ and $T_{c,out}$ are known but nothing about the hot streams is mentioned.

The same method as for past5 cannot be reused since $T_{h,in}$ and $T_{c,in}$ are unknown in this scenario.

Thus it is first needed to determine $T_{h,in}$ from the Past2 ∆T (assumed here at 5K for simplicity). This gives $T_{past,c} = 70°C$ which will be $T_{h,in}$ of past1.

Another important assumption in this scenario is that for past1 $T_{c,out}=T_{past-cent,out} \ne 60°C$. Indeed, this temperature will instead be determined from the ∆Tmin optimization of past1.
...
The first graph shows that dT is on the left side, thus the corresponding equations presented at the beginning of this section are used.

The optimisation gives a $\Delta T_{min}$ which varies between Germany and France. This leads to different CAPEX and OPEX for both countries.

For the OPEX, the costs of past3 (heating) and past 4 (cooling) are to be included. 

The Past 3 equation will be the following: $Q_{past3} = m_{hot}*cp_{hot}*(T_{past2,out}-348)$ where $m_{hot}$ is the flow coming out of past2. 

The Past 4 equation will be the following: $m_{hot}*cp_{hot}*(T_{hot,out,past1}-277)$.

Of course the heating OPEX (past 3) will use gas costs whereas the cooling OPEX (past 4) will use electricity costs and consider a COP of 3 as in the energy bill section. Both processes again assume the same operating time as for the Energy Bill.

#### Past 5 optimization

Now the same as above is performed for past 5. This time because both past 1 and past 5 are connected it is assumed that $T_{cold,in,past5}=T_{hot,out,past1}$
...
The first graph shows that dT is on the right side, thus the corresponding equations presented at the beginning of this section will be used.

For the OPEX the costs of past 6 (heating) and past 7 (cooling) must be included. 

The Past 6 equation will be the following: $Q_{past6} = m_{hot}*cp_{hot}*(T_{hot,in,past5}-T_{cold,out,past5})$ where $m_{hot}$ is the flow coming out of past5. 

The Past 7 equation will be: $m_{hot}*cp_{hot}*(T_{hot,out,past1}-277)$

Of course the heating OPEX (past 6) will use gas costs whereas the cooling OPEX (past 7) will use electricity costs and consider a COP of 3 as in energy bill. Both processes again assume the same operating time as for the Energy Bill.

The optimisation gives a $\Delta T_{min}$ for past 5 of 6.3K leading to an area of 5.21$m^2$ and a larger TOTEX in Germany than France because of the higher price of electricity in Germany (both CAPEX are the same but the OPEX varies). 

Compared to the case where only past5 was optimized, it presents a higher $\Delta T_{min}$ but a lower Area and thus lower costs. Considering only the costs of past5, it may seem more interesting to optimize past1 and past5 simultaneously. However one has to look at the whole picture and not forget that past1 was also optimized (thus mmust consider the total costs of both past1 and past5).

#### Both costs combined 

Finaly, let us combine both costs from past 1 and past 5 to get the overall CAPEX, OPEX, and TOTEX for the two pasteurizers. These total costs will then be compared to the energy bill without optimization over $\Delta T_{min}$. 
...
### Pasteurization with Recovery HEX

The only possibility to recover heat in the pasteurisation section is by adding a new heat exchanger to preheat the thickener with the ouput milk flow. The following Figure illustrates the location of this new heat exchanger.

![Pasteurisation](Figures/Recovery.jpg)

Assumption: crpast_b is fixed to 75°C. This assumption makes it so that a reoptimization of past5 is not necessary.

Available information:

- $m_{past,d1} = 7.52 kg/s$
- $m_{thickener} = 0.07 kg/s$
- $T_{past,d1} = 10.8°C$
- $T_{thickener,in} = 4°C$

The goal is to increase the thickener's temperature when entering the mixer thanks to the heat excess of the pasteurized milk going out of the system. 

Considering the huge mass flow difference, it can be assumed that $T_{past,d1} \approx T_{past,d2} \approx T_{thickener,out} = 10.8°C$. 

To compute the cp of the thickener, the function mixture is used, assuming the thickener is composed of 100% fat. 

Thanks to this data, the heat exchanged in the recovery HEX can be computed.

```{python Qrecovery, echo = T, eval = T}
from pyxosmose.state import State
from codes_01_energy_bill.coolprop_functions import mixture

m_thickener = 0.07
T_thickener_in = 277
T_thickener_out = 283.8

State = mixture(T = T_thickener_out, P = 100000, frac_water = 0, frac_fat = 1)
cp = State["cpmass"]

Q = m_thickener * cp * (T_thickener_out - T_thickener_in)
#print('Q_recovery =', round(Q, 3), 'W')
```
The heat exchanged is thus, of 1kW. 

The next step is to compute the output temperature of the mixer.

- $m_{crpast,x} = m_{thickener} + m_{crpast,a} = 0.07 + 0.48 = 0.55 kg/s$
- $T_{crpast,a} = 65.9°C$
- $T_{thickener} = 10.8°C$

For these calculations the same equations as the ones used for non-isothermal mixers in the mass & energy balance section are reused.
```{python Tcrpastx, echo = T, eval = T}
from pyxosmose.state import State
from codes_01_energy_bill.coolprop_functions import mixture
...
T_cr_past_a = 338.9
cp_cr_past_a = 3.5
m_cr_past_x = m_thickener + m_cr_past_a
cp_cr_past_x = 3.5

T_cr_past_x = (m_thickener * cp_thickener * T_thickener + m_cr_past_a * cp_cr_past_a * T_cr_past_a) / (m_cr_past_x * cp_cr_past_x)
#print('T_cr_past_x =', round(T_cr_past_x, 3), 'K')
```

This leads to $T_{crpast,x}=65.9°C$. Compared to the original pasteurisation process, there is thus an increase of 12.40°C. This will reduce the heat required in the past6 process.
```{python TOTEX_Pastrecovery, echo = T, eval = T}
Q=1006 #W
alpha= 1000 #W/m^2°C, assumption based on alpha_H20
U = alpha/2
LMTD = 283.8 #because T_hot,in = T_hot,out = T_cold,out = 283.8°C so applying LMTD formula would make no sense
Area = Q/(U*LMTD)
...
#Only past6 is impacted by the addition of the new HX since past 4 will still receive a flow of 10.8°C as in the original case
m_crpastx = 0.55
cp_crpastx = 3500
T_crpastx = 338.9
T_crpastb = 348
Qpast6 = m_crpastx * cp_crpastx * (T_crpastb - T_crpastx) #Q in W since cp in J/kgK
OP_steam_past6_fr = gas_fr * Qpast6/1000 * top 
OP_steam_past6_ger = gas_ger * Qpast6/1000 * top

OPEX_fr = OP_steam_past6_fr 
OPEX_ger = OP_steam_past6_ger   
...
area_fr_opt_hx = Area
area_ger_opt_hx = Area
CAPEX_fr_opt_hx = CAPEX
CAPEX_ger_opt_hx = CAPEX
OPEX_fr_opt_hx = OPEX_fr
OPEX_ger_opt_hx = OPEX_ger
TOTEX_fr_opt_hx = TOTEX_fr
TOTEX_ger_opt_hx = TOTEX_ger

HX_opt_fr = pd.DataFrame({'Area_fr[m^2]': area_fr_opt_hx, 'CAPEX_fr[€/yr]': CAPEX_fr_opt_hx, 'OPEX_fr[€/yr]': OPEX_fr_opt_hx, 'TOTEX_fr[€/yr]': TOTEX_fr_opt_hx}, index=[0])
HX_opt_fr = HX_opt_fr.round(2)
HTML(HX_opt_fr.to_html(index=False))

HX_opt_ger = pd.DataFrame({'Area_ger[m^2]': area_ger_opt_hx, 'CAPEX_ger[€/yr]': CAPEX_ger_opt_hx, 'OPEX_ger[€/yr]': OPEX_ger_opt_hx, 'TOTEX_ger[€/yr]': TOTEX_ger_opt_hx}, index=[0])
HX_opt_ger = HX_opt_ger.round(2)
HTML(HX_opt_ger.to_html(index=False))
```


### Cleaning in Place

The next part focuses on optimizing the heat recovery of the cleaning in place process. After observing the heat exchangers in this process one can notice that there is one used to heat thanks to the use of steam between flows CIP2 and CIP3/10, and one heat exchanger used to cool down with glycolic water between flows CIP6 and CIP7.

The last one could be used to preheat the flow cip1 which is currently at 25°C and which needs to be heated to 60°C before entering the HX between CIP2 and CIP3/10.

NB: The same could have been done by preheating the CIP2 to make it closer to CIP3's temperature of 80°C. However, the gap would have been lowered (only 20°C difference compared to 35°C for the chosen solution), thus making the heat recovery less efficient.
 
The following relation between the flow's temperatures had been determined in chapter 2:

- $T_{cip6}=75°C > T_{cip1}=25°C$

The new HX will be placed in such a way that CIP6's flow will first be used to preheat CIP1 to CIP1' (with $T_{cip1'} > T_{cip1}$) and the exiting flow named CIP6' (such that $T_{cip6'}< T_{cip6}$) will then be used to be cooled to where CIP6 previously was. 

Thus, the system will require less cooling since $T_{cip6'}< T_{cip6}$, as well as also requiring less heating since $T_{cip1'} > T_{cip1}$.
...
In this scenario the OPEX is dependent on the the cooling and heating HX of the CIP process. 

First, new flows are redetermined for m_cip1 and m_cip5 such that $m_{cip1} = m_{cip2}*\frac{T_{cip2}-T_{cip5}}{T_{cip1}-T_{cip5}}$ and $m_{cip5}=m_{cip2}-m_{cip1}$, where $T_{cip1}$=$T_{cip2}$=$T_{cold,out,newHX}$ is the new exiting temperature, and cip1' is hotter than cip1 from the task2.

From this we will obtain that $Q_{steam} = m_{cip2} \cdot cp1 \cdot (353-T_{cip2})$ and $Q_{cooling} = m_{hot,newHX} \cdot cp_{hot,newHX} \cdot (T_{hot,out,newHX}-303)$ 


It can be observed that the optimum dT is 24°C, which brings CIP1' to ($75-24=51°C$) instead of 25°C. Thus compared to the case without optimization CIP2 will be cooler than 60°C and by consequence the heat exchanger between CIP2 and CIP3/10 will require more steam to heat up until 80°C. 

This doesn't seem to be efficient. This will be verified by comparing the costs with the Energy Bill.

### Comparison with non-optimalized case:

In each case presented above, an optimized $\Delta T_{min}$ has been selected to reduce the cost of the considered system. But even by selecting this minimum cost, is this heat recovery optimization really worth it? To verify this, a comparison will be made with the non-optimized cases from the Energy Bill.

The following table summarizes the cost for the optimized (with heat recovery) and non-optimized (without heat recovery) OPEX for each process. The costs are given for both France and Germany. Only the OPEX are considered since in the Energy Bill only the OPEX were computed, making it not possible to compare CAPEX and TOTEX.
...
<tbody>
  ...
  <tr>
    <td class="tg-c3ow" colspan="3">Scenario 3: Pasteurization with new HX</td>
  </tr>
  <tr>
    <td class="tg-0pky">France</td>
    <td class="tg-c3ow">23707.71</td>
    <td class="tg-fpwh">11662.45</td>
  </tr>
  <tr>
    <td class="tg-0pky">Germany</td>
    <td class="tg-c3ow">23707.71</td>
...
From this table it appears optimizing $\Delta T_{min}$ for past5 HX only is a good initiative since it reduces the OPEX by 25% in France and 21% in Germany. 
On top of that, an additional HX could be placed to heat up the thickener before the past6 HX and lead to a 50% reduction of the OPEX in France and Germany.
Finally, in the CIP process a HX could be added but only in Germany to preheat the incoming flow and reduce the OPEX by 7%.

## Minimum energy requirement

The minimum energy requirement, including both heating and cooling demand, can be determined through the application of a pinch analysis. <!--In the Survival Guide, you can find an example of using QRosmose to conduct pinch analysis for a brewery plant. For your project, you are expected to employ the same methodology to analyze the dairy factory. While more detailed tutorials will be provided during the semester, the TA team will also be available to assist you as needed.-->
...
To achieve this, H-T diagrams are employed to determine the minimal temperature differences between hot and cold exchanges, defining it as the pinch point. Subsequently, the process is partitioned into cold and hot areas, both in relation to the pinch point. 
...
This methodology is thus known as the pinch point analysis.

### Analysis Results

In this section the results of pinch point analysis are presented. First the Composite Curves (Fig. 1) and the Grand Composite Curve (Fig. 2) are shown. 

From the composite curves it is then possible to extrapolate the energy requirements, the heat recovery and the utilities. By separating the graph in three portions, with two green dotted lines, the boundaries of the utilities and the heat recovery are shown.

The graph of the grand composite curve can then be used to determine the temperature of the pinch points. These are the points where the heat load is equal to 0, meaning that the heats of the corrected cold and hot streams are the same in these points.

The composite curves can be seen in the following Figure:
...
In the next Tables the temperature of the pinch point and the energy requirements and heat recovery are presented:
...
It can be verified that the heat load at this point is indeed (approximately) 0 W by extracting its value at this points:
...
It is also possible to compute the energy requirements and maximum heat recovery:
```{python, echo = T, eval = T}
