...
![HENbeforedesign](Figures/HENflowsbef.jpg)
...
- E1 is placed between F and B1 and $Q_{E1} = mcp_{B1}*(T_{in,B}-T_{pinch}) = 83.5 kW$
...
- E2 is placed between A1 and B2 and $Q_{E2} = mcp_{B2}*(T_{in,B}-T_{pinch}) = 59.5 kW$
...
- E7 is placed between F1 and H and $Q_{E7} = mcp_{F1}*(T_{pinch} - T_{in,F}) = 58.5 kW$
...
- E8 is placed between F2 and G and $Q_{E8} = mcp_{F2}*(T_{pinch} - T_{in,F}) = 25 kW$
...
![HENafterdesign](Figures/HENflowsaft.jpg)
...
The new design Operating price will then be: 776.9k€/yr and the new Investment price will decrease by: 26.9k€/yr.

It is overall slightly lower (+23.7 - 26.9 = -3.2k€/yr gained) than the original design. Thus, it may be useful to remove E4 from the HEN design.
...
The new design Operating price will then be: 746.7 and the new Investment price will decrease by nothing since E9 was already removed from the design but now it is done in a more justified way.   

It is slightly than the original design. Thus, it may be useful to remove E9 from the HEN design and apply this loop.
