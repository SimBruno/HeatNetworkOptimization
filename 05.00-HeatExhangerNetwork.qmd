...
Now that the pinch point has been determined (task 5) via the Grand Composite Curve it is possible to design the Heat Exchanger Network (HEN) for the plant. First the penalizing exchangers will be determined (task 6) and then the HEN will be designed and optimized (task 7). 
...
Furthermore, it is possible to calculate the penalty caused by each penalizing heat exchanger, as it is the amount of heat utilized by said exchanger.

As a final note, the wastewater and sludge production section will not be considered for this section as it does not utilize any heat exchangers.
...
In this design, from the results of task 5 and 6, the pinch point is at 67.5 °C for a $\Delta T_{min}$ of 5K.

The flows will only be taken from the pasteurization, cleaning in place, hot water and cold storage processes.

This said, a constraint will be that only penalizing flows will be considered. These are the ones where a cold utility is above the pinch point, a hot utility is below the pinch point or a flow crosses or starts/ends at the pinch point.
...
From the list of the flows determined in task 5, the following flows are penalizing and will be considered for the HEN design:
...
It may be noticed that in the pasteurization process the flow crossing Past1 HX to heat up from 4°C to 60°C was not considered. There was an attempt to perform the HEN design with this flow but it introduced a too large unbalance in the $\sum mcp_{cold}$ vs $\sum mcp_{hot}$ below the pinch (the sum was higher for cold streams instead of hot streams). Thus, this flow has been removed from the HEN design. This is not a problem since this flow, located below the pinch, can be entirely heated up by the part of hot flow B below the pinch. Thus there is no need to cross the pinch point to heat up this flow and it is not penalizing.

Another thing that is important to be noted is that for the CIP process, the optimized design from task4 with an additionnal HX to heat up the incoming CIP1 flow was used.
...
Now that the cold and hot flows are defined, the design can be determined. 
...
- When all flows have been connected with HX, add Cold Utilities and Hot Utilities to reach the target temperatures of the respective hot and cold flows

Let us start with the design above the pinch point (recall $T_{pinch}=67.5°C$).

1. Step 1:

- $N_{cold} = 3$ and $N_{hot} = 4$ thus a splitting is needed to respect the 1st feasibility rule.
...
- Two biggest mcp flows are F and B. However $mcp_{cold} \leq mcp_{hot}$ 
- Splitting of flow B into B1 and B2 such that $mcp_{B1} =  16.7 kW/K$ and $mcp_{B2} = 11.9 kW/K$. 
- Now the 2nd feasibility rule is respected.
...
- The two biggest mcp flows are A1 and B2 and the 2nd feasibility rule is respected.
- E2 is placed between A1 and B2 and $Q_{E2} = mcp_{B2} \cdot (T_{in,B}-T_{pinch}) = 59.5 kW$

3. Step 3:

- $N_{cold} = 3$ and $N_{hot} = 3$ thus no splitting is needed.
- The two biggest mcp flows are A2a and H and the 2nd feasibility rule is respected.
- E3 is placed between A2a and H and $Q_{E3} = Q_{H} = 58.5 kW$

4. Step 4:

- $N_{cold} = 2$ and $N_{hot} = 2$ thus no splitting is needed.
- The two biggest mcp flows are A2b and G and the 2nd feasibility rule is respected.
- E4 is placed between A2b and G and $Q_{E4} = Q_{G} = 25.1 kW$

5. Step 5:

- $N_{cold} = 1$ and $N_{hot} = 1$ thus no splitting is needed.
- The two biggest mcp flows are D and E and the 2nd feasibility rule is respected.
...
- HU1 is placed after the splitting of A and $Q_{HU1} = Q_{A} - Q_{E2} -  Q_{E3} - Q_{E4} = 142.6 kW$
...
We will now continue with the design below the pinch point ($T_{pinch}=67.5°C$).

1. Step 1:

- First it can be noticed that $\sum mcp_{hot} \leq \sum mcp_{cold}$. This is due to the 1.6 kW/K of the flow C, thus no matter how we connect the flows the 2nd feasibility rule will never be respected. 
We will thus remove the flow C below the pinch from all of the following calculations at first, and then handle it at the end.
- $N_{hot} = 4$ and $N_{cold} = 3$ (w/o flow C) thus no splitting is needed.
- The two biggest mcp flows are A and B and the 2nd feasibility rule is respected.
- E6 is placed between A and B and $Q_{E6} = Q_{A} = 142.9 kW$

2. Step 2:

- $N_{hot} = 3$ and $N_{cold} = 2$ (w/o flow C) thus no splitting is needed.
- The two biggest mcp flows are F and H and the 2nd feasibility rule is not respected (i.e., $mcp_{hot} \leq mcp_{cold}$).
...
- $N_{hot} = 2$ and $N_{cold} = 2$ (w/o flow C) thus no splitting is needed.
- The two biggest mcp flows are F2 and G and the 2nd feasibility rule is respected.
- E8 is placed between F2 and G and $Q_{E8} = mcp_{F2} \cdot (T_{pinch} - T_{in,F}) = 25 kW$

4. Step 4:

- $N_{hot} = 1$ and $N_{cold} = 1$ (w/o flow C) thus no splitting is needed.
- The two biggest mcp flows are D and E and the 2nd feasibility rule is respected.
- E9 is placed between D and E and $Q_{E9} = Q_{D} = 16.36 kW$

5. Step 5:

- We will now handle flow C. It is possible to use the rest of heat from flow B to heat up flow C. 
...
Here is the final HEN design (note that additionnal temperatures were indicated on some flows to help understand what temperature gap the utilities have to fill):

![HEN after design](Figures/HENflowsaft.jpg)

It appears that the method used here doesn't prevent impossible HX. Indeed, it is possible to see that E9 and E10 are impossible as the temperature of the cold flow is higher than the temperature of the hot flow. 

Because of this infeasibility E9 and E10 will be removed from the following economic analysis. Further on in the report, with the help of the loop method, it will be possible to remove E9 from the design.

### Economic Analysis of the First Heat Exchanger Network

The economic analysis of the heat exchanger network is done by calculating both the capital cost and the operating cost.

The equations for the Investment Cost and Annualized Investment Cost can be found in task 4 where the $\Delta T_{min}$ optimization was done. All parameters and assumptions remain the same, only the temperatures are different and come from the HEN design above.
...
The operating cost of the added Cold and Hot Utility is calculated by using the following equations:
...
The table below shows the operating costs of the added utilities:
...
The small difference between the CU1 on the graph and the CU1 on the table is due to the removal of E10 HX from the design while creating the table for the reasons explained above.
...
The payback time is computed with the following equation:
...
The system is not economically feasible as the payback time is too long. The choice of $\Delta_{min}$ is not optimal and the HEN design contains some flaws. 
...
Different paths can be followed to remove HX from the original HEN design, however, only one will be detailed here. 

The considered path starts from the HU1 on flow A and ends on the CU4 on flow H after successively passing through E4, E8 and E7. The goal is to remove E4 which exchange 25.1kW between A2b and G. By doing so, one has to also change the rest of the HEN design to compensate the loss of the heat exchanger:
...
Overall the path method allows to gain 3.1k€/yr compared to after the HEN design. However the payback time would still be way too high because of the increase in Operating Costs.
...
Since it was said that E9 was unfeasible we will try to find out whether we can remove it from the design by using the loop method, thus we will focus on the loop between D and E.

The loop will pass through E5 and E9. The goal is to remove E9 which exchanges 16.36 kW between D and E. By doing so, one also has to change the rest of the HEN design to compensate the loss of the heat exchanger:

- E5 will gain 16.36 kW and exchange 25.96 kW.

This will lead to an increase of $T_{cold,E5,out}$ on the flow D which then becomes 354 K instead of 350.5 K. 

- HU2 will be removed since the maximum temperature of the flow D, now 354°C, is reached.
...
Compared to the path method, the loop method allows to reduce the operating costs and thus reduce the payback time by more than 20 years. This said, the payback time is still very high .
